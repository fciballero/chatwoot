...
# Stub class for TiendanubeAPI response
class TiendanubeAPIResponse
  attr_reader :body
...
RSpec.describe 'Tiendanube Integration API', type: :request do
  ...
  describe 'POST /api/v1/accounts/:account_id/integrations/tiendanube/auth' do
    let(:shop_domain) { 'tiendatest176.mitiendanube.com' }

    context 'when it is an authenticated user' do
      it 'returns a redirect URL for Tiendanube OAuth' do
  ...
  describe 'GET /api/v1/accounts/:account_id/integrations/tiendanube/orders' do
    before do
      create(:integrations_hook, :Tiendanube, account: account)
    end

    context 'when it is an authenticated user' do
      # rubocop:disable RSpec/AnyInstance
      let(:Tiendanube_client) { instance_double(TiendanubeAPI::Clients::Rest::Admin) }

      let(:customers_response) do
        instance_double(
          TiendanubeAPIResponse,
      ...
      before do
        allow_any_instance_of(Api::V1::Accounts::Integrations::TiendanubeController).to receive(:Tiendanube_client).and_return(Tiendanube_client)

        allow_any_instance_of(Api::V1::Accounts::Integrations::TiendanubeController).to receive(:client_id).and_return('test_client_id')
        allow_any_instance_of(Api::V1::Accounts::Integrations::TiendanubeController).to receive(:client_secret).and_return('test_client_secret')

        allow(tiendanube_client).to receive(:get).with(
  ...
  describe 'DELETE /api/v1/accounts/:account_id/integrations/tiendanube' do
    ...
    context 'when it is an authenticated user' do
      it 'deletes the Tiendanube integration' do
        expect do
