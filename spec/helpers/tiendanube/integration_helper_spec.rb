...
RSpec.describe Tiendanube::IntegrationHelper do
  ...
  describe '#generate_tiendanube_token' do
    ...
    it 'generates a valid JWT token with correct payload' do
      token = generate_tiendanube_token(account_id)
    ...
    context 'when client secret is not configured' do
      ...
      it 'returns nil' do
        expect(generate_tiendanube_token(account_id)).to be_nil
      end
    end
    ...
    context 'when an error occurs' do
      ...
      it 'logs the error and returns nil' do
        expect(Rails.logger).to receive(:error).with('Failed to generate Tiendanube token: Test error')
        expect(generate_tiendanube_token(account_id)).to be_nil
      end
    end
  end
  ...
  describe '#verify_tiendanube_token' do
    ...
    it 'successfully verifies and returns account_id from valid token' do
      expect(verify_tiendanube_token(valid_token)).to eq(account_id)
    end
    ...
    context 'when client secret is not configured' do
      ...
      it 'returns nil' do
        expect(verify_tiendanube_token(valid_token)).to be_nil
      end
    end

    context 'when token is invalid' do
      it 'logs the error and returns nil' do
        expect(Rails.logger).to receive(:error).with(/Unexpected error verifying Tiendanube token:/)
  ...
  describe '#client_id' do
    it 'loads client_id from GlobalConfigService' do
      expect(GlobalConfigService).to receive(:load).with('TIENDANUBE_CLIENT_ID', nil)
      client_id
    end
  end
  ...
  describe '#client_secret' do
    it 'loads client_secret from GlobalConfigService' do
      expect(GlobalConfigService).to receive(:load).with('TIENDANUBE_CLIENT_SECRET', nil)
      client_secret
    end
  end
end
